...
impl<'a> PrattParser<'a> {
    ...
    pub fn parse_expression(&mut self, min_prec: u8) -> Result<Expression, ParseError> {
    ...
    fn parse_prefix(&mut self) -> Result<Expression, ParseError> {
        match self.current().clone() {
            ...
            Token::FnUuid => {
                self.advance();
                self.parse_fn_call("uuid")
            }
            Token::FnNow => {
                self.advance();
                self.parse_fn_call("now")
            }
            Token::FnToday => {
                self.advance();
                self.parse_fn_call("today")
            }
            Token::FnCount => {
                self.advance();
                self.parse_fn_call("COUNT")
            }
            Token::FnSum => {
                self.advance();
                self.parse_fn_call("SUM")
            }
            Token::FnAvg => {
                self.advance();
                self.parse_fn_call("AVG")
            }
            Token::FnMax => {
                self.advance();
                self.parse_fn_call("MAX")
            }
            Token::FnMin => {
                self.advance();
                self.parse_fn_call("MIN")
            }
    ...
    fn parse_infix(&mut self, left: Expression, prec: u8) -> Result<Expression, ParseError> {
        match self.current().clone() {
            // Binary operators
            Token::Eq | Token::NotEq | Token::Gt | Token::Lt | Token::Gte | Token::Lte => {
            ...
            Token::Comma => {
                self.advance();
                let right = self.parse_expression(Precedence::And as u8)?;
                Ok(match left {
            ...
            Token::Pipe => {
                self.advance();
                let right = self.parse_expression(Precedence::Or as u8)?;
                Ok(match left {
    ...
    fn parse_identifier_expr(&mut self, name: String) -> Result<Expression, ParseError> {
        // No dot — simple field
        if self.current() != &Token::Dot {
    ...
    fn parse_fn_call(&mut self, name: &str) -> Result<Expression, ParseError> {
        self.expect(Token::LParen)?;
        let mut args = vec![];
    ...
    fn expect_distance(&mut self) -> Result<coffeeql_lexer::token::Distance, ParseError> {
        match self.current().clone() {
            Token::Distance(d) => {
                self.advance();
                Ok(d)
    ...
    fn expect_duration(&mut self) -> Result<coffeeql_lexer::token::Duration, ParseError> {
        match self.current().clone() {
            Token::Duration(d) => {
                self.advance();
                Ok(d)
